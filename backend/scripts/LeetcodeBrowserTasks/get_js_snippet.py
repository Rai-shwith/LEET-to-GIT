def generate_browser_snippet(websocket_domain: str, connection_id: str) -> str:
    return f"""const connection_id = '{connection_id}';
...
function getExtension(language) {{
  const extensionMap = {{
    python3: "py", python: "py", pandas: "py", java: "java", c: "c", cpp: "cpp",
    csharp: "cs", javascript: "js", typescript: "ts", ruby: "rb", swift: "swift",
    go: "go", kotlin: "kt", scala: "scala", rust: "rs", php: "php", mysql: "sql",
    bash: "sh", perl: "pl", haskell: "hs", dart: "dart", racket: "rkt", elixir: "ex",
    erlang: "erl", "objective-c": "m", matlab: "m", fsharp: "fs", lua: "lua",
    groovy: "groovy", "vb.net": "vb", fortran: "f90", pascal: "pas", julia: "jl",
    prolog: "pl", scheme: "scm", cobol: "cbl", solidity: "sol",
  }};
...
async function fetchSubmissions() {{
  ...
  while (hasNext) {{
    try {{
      const response = await fetch(`https://leetcode.com/api/submissions/?offset=${{offset}}`);
      if (!response.ok) {{
  ...
  const socket = new WebSocket(`{websocket_domain}${{connection_id}}`);
  socket.onopen = () => {{
